        ...
        add: function(){
            ...
            if(!this.checkAllFields(this.interest)){
                ...
                if(confirm("Are You Sure?")){
                    ...
                    vm.interest = {...vm.interest, date: String(vm.interest.date).substring(8,10), principle: 0, lastMonthPaid: false}
                    db.collection("Interests").add(vm.interest).then(()=>{
                    db.collection("Members").doc(this.interest.mid).collection("Interests").add({
                        amount: vm.interest.amount,
                        rate: vm.interest.rate,
                        date: String(vm.interest.date).substring(8,10)
                    }).then(()=>{
                        db.collection("Members").doc(vm.interest.mid).update({
                            interestDate: String(vm.interest.date).substring(8,10),
                            interestDue: 0,
                            interestAmount: 0,
                        }).then(()=>{
