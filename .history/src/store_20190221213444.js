...
export default new Vuex.Store({
    ...
    actions: {
        ...
        checkforMember(state, {mid}){
            return new Promise((resolve, reject)=>{
                db.collection("Members").doc(mid).get().then((doc)=>{
                    if(doc.exists){
        ...
        updateDailyBook(state, {data}){
            return new Promise((resolve, reject)=>{
                db.collection('DailyBook').doc(data.id).update(data).then(()=> resolve("Completed"))
            })
        }
    }
})