<template>
    <div class="groups">
        ...
        <table>
            <tr>
                <td style="width: 15%">Name : {{message}}</td>
...
export default {
    ...
    methods: {
        ...
        add(){
            ...
            console.log(String(this.data.today).replace('/', '-'));
            if(!this.checkAllFields(this.data)){
                vm.showIncompleteAlert("Fields Incomplete");
            }else{
                if(confirm('Are You Sure?')){
                    db.collection("DailyBook").doc(String(vm.data.today).replace(/\//g, 'message')).collection("data").add(vm.data).then(()=>{
                        vm.showSuccessAlert();
                    })
                }  
            }
    ...
    computed: {
        computeDate(){
            ...
            return this.data.today = String(this.data.today).substring(8,16)
        }
    }
}
</script>
