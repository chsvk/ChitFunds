<template>
    <div class="groups">
        ...
        <div class="select">
            ...
            <h3 style="margin-left: 1em"> Total On {{formatDate(dateSelected)}}: <span style="font-weight: 100; margin-left: 0.6em">  {{getTotal()}} </span> </h3>
        </div>
        <table>
            <tr>
                <th>Date</th>
...
export default {
    ...
    methods: {
        ...
        fetchData(){var vm = this;
            db.collection("DailyBook").where("today", "==", vm.formatDate(this.dateSelected)).onSnapshot(function(query){
                vm.groups = [];
