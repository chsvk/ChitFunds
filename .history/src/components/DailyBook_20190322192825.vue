    ...
    <div class="groups">
        ...
        <table>
            <tr>
                ...
                <td>Today-- {{formatDate(new Date())}}</td>
...
export default {
    ...
    methods: {
        ...
        getTodayTotal(){
            var Total = 0;
            firebase.firestore().collection("DailyBook").where("today", "==", this.formatDate(new Date())).onSnapshot((doc)=>{
                doc.forEach((doc)=>{
                    Total = Total + Number(doc.data().chitValue)
                })
            })
            return Total
        }
