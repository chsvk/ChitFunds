      ...
      <div class="modal-wrapper">
        ...
        <table>
            ...
            <tr v-for="(g,i) in groups" :key="i">
                <td><p>{{Month}} ({{g.monthNumber}} Months)</p></td>
                <td><p>{{g.name}}</p></td>
                <td><p>{{g.value}}</p></td>
                <td><p>{{g.value/g.months}}</p></td>
                <td v-if="g.monthNumber==0">{{g.value/g.months}}</td>
                <td v-else><p>{{g.netInstallment}}</p></td>
...
export default {
    ...
    methods: {
        ...
        add: function(){
            ...
            if(confirm("Are You Sure?")){
                var c_NetInstallement;
                var todaysDate = String(this.getDate()).split('/').join('-')
                if(vm.calculateNetInstallement){
                    c_NetInstallement = (vm.temporary.value/vm.temporary.months) - ((vm.auctionAmount-((vm.commissionPercentage/100)*vm.temporary.value))/vm.temporary.months)
                }else{
                    c_NetInstallement = vm.temporary.value/vm.temporary.months;
                }
                // Step 1: Updating Net Installement
                db.collection("Groups").doc(vm.temporary.name).update({
                    netInstallment: c_NetInstallement,
                    monthNumber: ++vm.temporary.monthNumber
                }).then(()=>{
                    db.collection("Groups").doc(vm.temporary.name).collection("Members").get().then((query)=>{
                        query.forEach((doc)=>{
                            var updatingDue;
                            db.collection("Members").doc(doc.id).get().then((doc)=>{
                                updatingDue = doc.data().due;
                            }).then(()=>{
                                db.collection("Members").doc(doc.id).update({
                                    due: updatingDue + c_NetInstallement
                                }).then(()=>{
                                    db.collection("Members").doc(vm.mid).collection("Groups").doc(vm.temporary.name).update({
                                        ChitLifted: true,
                                        LiftedDate: todaysDate
                                    }).then(()=>{
                                        db.collection('Groups').doc(vm.temporary.name).collection("Members").doc(vm.mid).update({
                                            ChitLifted: true,
                                            LiftedDate: todaysDate
                                        }).then(()=>{
        ...
        update: function(g){
            ...
            vm.temporary = this.groups[g];
             ...
             vm.showIncompleteAlert("WARNING!. THIS PROCESS CANNOT BE REVERSED!!")
        }
    },
    mounted(){var vm = this;
    var a = 0;
    ...
    vm.Month = this.getMonth();
        db.collection("Groups").onSnapshot((query)=>{
            vm.groups = [];
            query.forEach((doc)=>{
                ++a;
