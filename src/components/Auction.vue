      ...
      <div class="modal-wrapper">
        ...
        <table>
            ...
            <tr v-for="(g,i) in groups" :key="i">
                ...
                <td v-if="!calculateNetInstallement">{{g.value/g.months}}</td>
                <td v-else><p>{{(g.value/g.months)-((auctionAmount-((commissionPercentage/100)*g.value))/g.months)}}</p></td>
                <td><button @click="update(g)">Update</button></td>
            </tr>
        </table>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getToday: function(){
            ...
            return "09" + "th " + this.monthNames[mm].substring(0,3);
        },
        getMonth: function(){
            var today = new Date();
        ...
        add: function(){
            ...
            if(confirm("Are You Sure?")){
                ...
                db.collection("Groups").doc(temporary.name).collection("Members").doc(this.mid).update({
                netInstallment: i,
                chitLifted: true
                }).then(()=>{
                    db.collection("Groups").doc(temporary.name).get().then((query)=>{
                    query.forEach((doc)=>{
                        db.collection("Groups").doc(temporary.name).collection("Members").doc(doc.id).update({
                            netInstallment: i
                        })
                    })
                }).then(()=>{
                         db.collection("Auctions").doc(temporary.name).add({
